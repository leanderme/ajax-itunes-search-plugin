...
if( !class_exists('ITLPFramework') ){
    ...
    class ITLPFramework {
    	...
    	/**
         * Displays the "section_description" if speicified in $itlp_settings
         *
    	...
    	function section_intro( $args )
    	{
        	...
        	if(!empty($itlp_settings)){
        		foreach($itlp_settings as $section){
    	...
    	/**
         * Processes $itlp_settings and adds the sections and fields via the WordPress settings API
         */
    	function process_settings()
    	{
            global $itlp_settings;
        	if(!empty($itlp_settings)){
        	    usort($itlp_settings, array(&$this, 'sort_array'));
        		foreach($itlp_settings as $section){
    	...
    	/**
         * Usort callback. Sorts $itlp_settings by "section_order"
         * 
    	...
    	function generate_setting( $args )
    	{
        	...
        	$defaults = apply_filters( 'itlp_defaults', $defaults );
        	...
        	do_action('itlp_before_field');
        	do_action('itlp_before_field_'. $el_id);
    		switch( $type ){
    		    case 'text':
    		    ...
    		    case 'file':
                    $val = esc_attr($val);
    		        echo '<input type="text" name="'. $this->option_group .'_settings['. $el_id .']" id="'. $el_id .'" value="'. $val .'" class="regular-text '. $class .'" /> ';
                    echo '<input type="button" class="button itlp-browse" id="'. $el_id .'_button" value="Browse" />';
                    echo '<script type="text/javascript">
                    jQuery(document).ready(function($){
    		...
    		do_action('itlp_after_field');
        	do_action('itlp_after_field_'. $el_id);
    	}
        ...
        function settings()
        {
            do_action('itlp_before_settings');
            ?>
            <form action="options.php" method="post">
                <?php do_action('itlp_before_settings_fields'); ?>
                <?php settings_fields( $this->option_group ); ?>
        		<?php do_settings_sections( $this->option_group ); ?>
        		<p class="submit"><input type="submit" data-text-loading="Loading..." class="admin-btn save-btn" value="<?php _e( 'Save Changes' ); ?>" /></p>
			</form>
    		<?php
    		do_action('itlp_after_settings');
        }
    
    }   
}

if( !function_exists('itlp_get_option_group') ){
    /**
     ...
     */
    function itlp_get_option_group( $settings_file ){
        $option_group = preg_replace("/[^a-z0-9]+/i", "", basename( $settings_file, '.php' ));
        return $option_group;
    }
}

if( !function_exists('itlp_get_settings') ){
    /**
     ...
     */
    function itlp_get_settings( $settings_file, $option_group = '' ){
...
if( !function_exists('itlp_get_setting') ){
    /**
     ...
     */
    function itlp_get_setting( $option_group, $section_id, $field_id ){
...
if( !function_exists('itlp_delete_settings') ){
    /**
     ...
     * @param string optional "option_group" override
     */
    function itlp_delete_settings( $settings_file, $option_group = '' ){
        $opt_group = preg_replace("/[^a-z0-9]+/i", "", basename( $settings_file, '.php' ));
        if( $option_group ) $opt_group = $option_group;
